...
class Ability():
    ...
    def __init__(self, ability_name, ability_key, ability_description):
        ...
        raw_desc = ability_description.strip().split(". ")
        for sent in raw_desc:
            if sent == '':
                continue
            elif sent == raw_desc[-1]:
                self.description += f"{sent} "
            else:
                self.description += f"{sent}. "
...
class League:
    ...
    def update_data(self):
        ...
        passive_ability = raw_passive.find('h5').get_text()
        passive_symbol = raw_passive.find('h6').get_text()
        ...
        for raw_ability in raw_abilities:
            ability = raw_ability.find('h5').get_text()
            symbol = raw_ability.find('h6').get_text()
